...
async function main() {
  ...
  for (let i = 0; i < CLIENTS.length; i++) {
    const c = CLIENTS[i];
    const res = await api(token, 'POST', '/clients', c);
  ...
  for (let i = 0; i < 5; i++) {
    await api(token, 'POST', `/clients/${clientIds[i]}/claim`, { salesId: 'USR-002' });
  }
  console.log('  ✅ 已认领前5个客户\n');

  // ======= 阶段2: 创建主订单 =======
  console.log('📦 === 阶段2: 创建主订单 (10条) ===');
  const masterOrderIds: string[] = [];
  const masterOrderNos: string[] = [];
  for (let i = 0; i < ORDERS.length; i++) {
    const o = ORDERS[i];
    const res = await api(token, 'POST', '/orders/master', {
      ...o,
      customerId: clientIds[i],
      customerName: CLIENTS[i].name,
      salesPerson: '管理员',
    });
    const mid = res.data?.id || res.id;
    const mno = res.data?.orderNo || res.orderNo;
  ...
  const subOrderNos: string[] = [];
  for (let i = 0; i < ORDERS.length; i++) {
    const o = ORDERS[i];
    for (let j = 0; j < 2; j++) {
      const pkg = o.expressPackages[j];
      const res = await api(token, 'POST', '/orders/sub', {
      ...
      const sno = res.data?.subOrderNo || sid;
  ...
  for (let i = 0; i < ORDERS.length; i++) {
    for (let j = 0; j < 2; j++) {
      ...
      await api(token, 'POST', '/warehouse/inbound', {
  ...
  for (let i = 0; i < JOBS.length; i++) {
    ...
    const res = await api(token, 'POST', '/warehouse/units', {
      unitNo,
      unitType,
  ...
  for (let i = 0; i < JOBS.length; i++) {
    ...
    await api(token, 'POST', `/warehouse/units/${unitIds[i]}/load`, {
  ...
  for (let i = 0; i < unitIds.length; i++) {
    ...
    await api(token, 'POST', `/warehouse/units/${unitIds[i]}/seal`, { sealNo });
  ...
  for (let i = 0; i < JOBS.length; i++) {
    const j = JOBS[i];
    const res = await api(token, 'POST', '/jobs', j);
    const jno = res.data?.jobNo || res.jobNo;
    jobNos.push(jno);
  ...
  for (let i = 0; i < JOBS.length; i++) {
    await api(token, 'POST', `/jobs/${jobNos[i]}/bind-units`, {
      unitIds: [unitIds[i]],
    });
  ...
  for (let i = 0; i < 6; i++) {
    await api(token, 'PUT', `/jobs/${jobNos[i]}`, {
      status: 'IN_TRANSIT',
      currentPhase: 'IN_TRANSIT',
      originPhaseStatus: 'COMPLETED',
    });
    // 更新子订单状态
    await api(token, 'PUT', `/orders/sub/${subOrderIds[i * 2]}/status`, { status: 'IN_TRANSIT', currentNode: '已发运' });
    await api(token, 'PUT', `/orders/sub/${subOrderIds[i * 2 + 1]}/status`, { status: 'IN_TRANSIT', currentNode: '已发运' });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await api(token, 'PUT', `/orders/sub/${subOrderIds[i * 2]}/status`, { status: 'ARRIVED', currentNode: '已到达' });
    await api(token, 'PUT', `/orders/sub/${subOrderIds[i * 2 + 1]}/status`, { status: 'ARRIVED', currentNode: '已到达' });
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const sub1 = subOrderIds[i * 2];
    const sub2 = subOrderIds[i * 2 + 1];
    const res = await api(token, 'POST', '/delivery', {
  ...
  // 第1个配送单：已分配司机并签收
  if (deliveryIds[0]) {
    await api(token, 'POST', `/delivery/${deliveryIds[0]}/assign`, {
      driverId: 'DRV001',
      driverName: 'Mike Johnson',
      driverPhone: '+1-213-555-9001',
    });
    await api(token, 'POST', `/delivery/${deliveryIds[0]}/sign`, {});
    console.log('  ✅ 配送单1: 已签收');
    // 更新子订单为已配送
    await api(token, 'PUT', `/orders/sub/${subOrderIds[0]}/status`, { status: 'DELIVERED', currentNode: '已签收' });
    await api(token, 'PUT', `/orders/sub/${subOrderIds[1]}/status`, { status: 'DELIVERED', currentNode: '已签收' });
  }

  // 第2个配送单：已分配司机
  if (deliveryIds[1]) {
    await api(token, 'POST', `/delivery/${deliveryIds[1]}/assign`, {
      driverId: 'DRV002',
  ...
  for (let i = 0; i < ORDERS.length; i++) {
    ...
    for (let f = 0; f < numFees; f++) {
      ...
      const res = await api(token, 'POST', '/fees', {
        relatedType: 'ORDER',
  ...
  for (let i = 0; i < Math.min(15, feeIds.length); i++) {
    await api(token, 'POST', `/fees/${feeIds[i]}/approve`, {
      approver: '财务主管李四',
    });
  ...
  // 拒绝2笔
  if (feeIds.length > 15) {
    await api(token, 'POST', `/fees/${feeIds[15]}/reject`, {
      approver: '财务主管李四',
  ...
  if (feeIds.length > 16) {
    await api(token, 'POST', `/fees/${feeIds[16]}/reject`, {
      approver: '财务主管李四',
