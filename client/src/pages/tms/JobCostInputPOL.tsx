...
export const JobCostInputPOL: React.FC<{ businessMode?: 'ALL' | 'AIR' | 'SEA' }> = ({ businessMode = 'ALL' }) => {
  ...
  const [detailTitle, setDetailTitle] = useState('任务成本详情');
  ...
  const handleReject = async (record: CostRecord) => {
    try {
      await feeApi.reject(record.id, getCurrentUser(), '演示驳回：请补充费用说明');
      updateWorkflowState((prev) => {
        const nextDraft = { ...prev.draftOverrides };
        delete nextDraft[record.id];
        return {
  ...
  const handleOpenDetail = async (record: CostRecord) => {
    ...
    setDetailTitle(`任务成本详情 - ${record.jobNo}`);
  ...
  return (
    <div>
      <Alert
        ...
        message="演示规则：草稿可改可删，提交后待审锁定；审核通过后需先发起更改申请，并填写更改费用项目和原因。"
      />
      ...
      <Modal
        title={editingRecord ? `编辑草稿 - ${editingRecord.feeNo}` : '新增任务成本草稿'}
        open={modalVisible}
          ...
          <Form.List name="items">
            {(fields, { add, remove }) => (
              <div>
                ...
                <Button type="dashed" onClick={() => add({ changeType: 'UPDATE', currency: 'CNY', feeDirection: 'PAYABLE' })} block>
                  + 添加更改项目
                </Button>
              </div>
            )}
