...
import {
  ...
  PrinterOutlined, UploadOutlined, DollarOutlined
...
function mapServerJobToLocal(s: any): Job {
  ...
  return {
    ...
    containers: (s.shippingUnitIds || []).map((uid: string) => ({
      id: uid,
      containerNo: uid,
      type: s.transportType === 'AIR' ? 'ULD' : '40HQ',
      orderCount: 0,
      pieces: 0,
      weight: 0,
      volume: 0,
    ...
    operationLogs: [],
    isUrgent: false,
    operator: s.destOperator || s.originOperator || '',
...
export const DestJobList: React.FC<{ jobs?: any[]; businessMode?: 'ALL' | 'AIR' | 'SEA' }> = ({
  jobs: propsJobs = [],
  businessMode = 'ALL',
}) => {
  ...
  // 从API获取数据，或使用props传入的数据
  useEffect(() => {
    if (propsJobs && propsJobs.length > 0) {
      const source = businessMode === 'ALL'
        ? propsJobs
        : propsJobs.filter((j: any) => String(j.transportType || '').toUpperCase() === businessMode);
      const mapped = source.map(mapServerJobToLocal);
      setJobs(mapped);
      ...
      const fetchJobs = async () => {
        ...
        try {
          ...
          // 到达国办看已发运+已到达+到达国阶段的任务
          const destJobs = allJobs.filter((j: any) =>
            j.currentPhase === 'DESTINATION' || j.currentPhase === 'IN_TRANSIT' ||
            ['DEPARTED', 'IN_TRANSIT', 'ARRIVED', 'CLEARED', 'COMPLETED'].includes(j.status)
          );
          const lineFiltered = businessMode === 'ALL'
            ? destJobs
            : destJobs.filter((j: any) => String(j.transportType || '').toUpperCase() === businessMode);
          const data = lineFiltered.map(mapServerJobToLocal);
          setJobs(data);
  ...
  const arrivedCount = jobs.filter(j => j.status === 'ARRIVED').length;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record: Job) => (
        <Space vertical size={0}>
          <div>{record.containers?.length || 0}个运输单元</div>
          <div style={{ fontSize: 11, color: '#999' }}>
            {record.containers?.map(c => c.containerNo).slice(0, 2).join(', ') || '-'}
            {(record.containers?.length || 0) > 2 && '...'}
          </div>
        </Space>
      )
    },
    {
      ...
      render: (record: Job) => (
        <Space vertical size={0}>
          <div>{record.orderCount || 0}个订单</div>
          <div style={{ fontSize: 11, color: '#999' }}>
            {record.totalPieces || 0}件 / {(record.totalWeight || 0).toFixed(0)}kg
          </div>
        </Space>
      )
    },
