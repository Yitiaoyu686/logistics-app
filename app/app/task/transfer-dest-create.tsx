...
const DEST_WAREHOUSE_OPTIONS = [
  'LOS 总仓', 'ABV 分仓', 'ACC 分仓', 'IKEJ 站点', 'VI 站点', 'PH 站点',
];

const REASON_OPTIONS = [
  '总仓分拨', '站点补货', '库存调整', '紧急调货', '客户自提转仓', '其他',
];
...
const inferDirection = (from: string, to: string) => {
  ...
  return mainKeywords.some((k) => from.includes(k))
    ? 'MAIN_TO_BRANCH'
    : 'BRANCH_TO_MAIN';
};

export default function TransferDestCreateScreen() {
  ...
  const pickerOptions = pickerField === 'reason' ? REASON_OPTIONS : DEST_WAREHOUSE_OPTIONS;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const finish = () => router.replace('/task/transfer-dest' as any);
      if (Platform.OS === 'web') {
        window.alert('调拨单已创建，请前往到达国调拨列表管理');
        finish();
      } else {
        Alert.alert('创建成功', '调拨单已创建', [{ text: '确定', onPress: finish }]);
      }
    } catch (err: unknown) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.navBar}>
        ...
        <Text style={styles.navTitle}>创建到达国调拨单</Text>
        ...
        <ScrollView contentContainerStyle={styles.form}>
          ...
          <View style={styles.field}>
            ...
            <View style={styles.radioRow}>
              <TouchableOpacity style={styles.radioItem} onPress={() => setShippingMethod('DIRECT')}>
                ...
                <Text style={styles.radioLabel}>直达</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.radioItem} onPress={() => setShippingMethod('VIA_MAIN')}>
                ...
                <Text style={styles.radioLabel}>经总仓中转</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.tipCard}>
            ...
            <Text style={styles.tipText}>
              到达国调拨用于在 LOS/ABV/ACC 总仓与各站点间调配库存。司机、车辆信息在「执行发车」时填写。
            </Text>
          </View>
        </ScrollView>
            ...
            >
              {submitting ? (
                ...
                <>
                  ...
                  <Text style={styles.submitBtnText}>创建调拨单</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
