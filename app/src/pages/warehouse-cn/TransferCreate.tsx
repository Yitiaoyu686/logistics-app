...
const getDefaultWarehouse = (): WarehouseOption => {
  try {
    const user = JSON.parse(localStorage.getItem('user') || '{}')
    if (user.role === 'WAREHOUSE_US' || user.role === 'OPS_US') {
      return WAREHOUSE_OPTIONS[2] // 拉各斯主仓
    }
  } catch {}
  return WAREHOUSE_OPTIONS[0] // 广州仓
}

const TransferCreate = () => {
  const navigate = useNavigate()
  const [warehouses] = useState<WarehouseOption[]>(WAREHOUSE_OPTIONS)
  const [availableOrders, setAvailableOrders] = useState<OrderItem[]>([])
  const [fromWarehouse] = useState<WarehouseOption>(getDefaultWarehouse)
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        const res: any = await warehouseApi.listStock({ status: 'IN_STOCK' })
        const list = Array.isArray(res) ? res : (res?.data || [])
        const mapped: OrderItem[] = (list as any[]).map((r: any) => ({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await warehouseApi.createTransfer({
        fromWarehouse: fromWarehouse.code || 'CN',
      ...
      Toast.show({ content: '草稿已保存', icon: 'success' })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const created: any = await warehouseApi.createTransfer({
        fromWarehouse: fromWarehouse.code || 'CN',
        toWarehouse: toWarehouseCode,
        itemType: 'ORDER',
        remark,
      ...
      // 创建后立即提交（更新状态为 PACKED）
      const transferId = created?.data?.id || created?.id
      if (transferId) {
        await warehouseApi.updateTransfer(transferId, { status: 'PACKED' })
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        >
          <List>
            <List.Item extra={fromWarehouse.location ? `${fromWarehouse.name} (${fromWarehouse.location})` : fromWarehouse.name}>
              发出仓库
            </List.Item>
            <List.Item
              extra={
                selectedToWarehouse
                  ? (selectedToWarehouse.location ? `${selectedToWarehouse.name} (${selectedToWarehouse.location})` : selectedToWarehouse.name)
