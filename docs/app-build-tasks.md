...
| # | 页面 | 文件路径 | 状态 | 说明 |
|---|------|---------|------|------|
| 1 | 任务流首页 | app/(tabs)/tasks.tsx | ✅ 已完成 | 任务卡片列表+运营预告+筛选Tab |
| 2 | 扫码入库操作 | app/task/inbound.tsx | ⬜ 待开发 | 扫码→称重量方→生成子运单→打印面单→上架 |
| 3 | 库存列表+详情 | app/task/stock.tsx | ⬜ 待开发 | 搜索/扫码+筛选+详情+补打面单+改库位 |
| 4 | 装箱/执行出库 | app/task/packing.tsx | ⬜ 待开发 | 添加订单+绑定任务+执行出库(拖车/司机表单) |
...
| 8 | 配送签收/失败 | app/task/delivery.tsx | ⬜ 待开发 | 签收(拍照+COD)+失败(原因+拍照)+转自提 |
| 9 | 自提管理 | app/task/pickup.tsx | ⬜ 待开发 | 通知(短信)+核销(验提货码+COD) |

### 销售（5页）

| # | 页面 | 文件路径 | 状态 | 说明 |
|---|------|---------|------|------|
| 10 | 待办首页 | app/(tabs)/tasks.tsx | ✅ 已完成 | 同文件，按角色分支 |
| 11 | 客户列表+详情 | app/task/customer.tsx | ⬜ 待开发 | 搜索+列表+详情(联系人/地址/入仓号/订单) |
| 12 | 订单列表+详情 | app/task/order.tsx | ⬜ 待开发 | 筛选+列表+详情(物流时间线+费用+关联任务) |
| 13 | 运费试算 | app/task/quote.tsx | ⬜ 待开发 | 查询表单+结果+复制报价+分享 |
